    ...
    for i in range(image_num):
        ...
        if(config_test.get('whole_tumor_only', False) is True):
            ...
            if(pred1.sum() == 0):
                ...
                pred1_lc = ndimage.morphology.binary_closing(pred1, structure=struct)
                try:
                    _tmp = get_largest_two_component(pred1_lc, False, wt_threshold)
                    pred1_lc = _tmp
        ...
        if config_data['file_postfix'] == 'mha':
            # we need the VSD ID
            for k in img_names:
                if "Flair" in k:
                    patID = k.split("/")[-1].split(".")[-2]
                    break

            # we need patient folder ID (0301 -> 301)
            folderID = k.split("/")[-3][1:]

            # we need grade type
            grade_type = k.split("/")[-5]

            final_filename = "VSD.Seg_{}_{}.{}.mha".format(grade_type, folderID, patID)

            save_array_as_nifty_volume(final_label, os.path.join(save_folder, final_filename), img_names[0])
            print(final_filename)
        else:
            final_save_filename = save_folder + "/{0:}.nii.gz".format(temp_name)
            if not os.path.isdir("/".join(final_save_filename.split("/")[0:-1])):
                os.makedirs("/".join(final_save_filename.split("/")[0:-1]))
            save_array_as_nifty_volume(final_label, final_save_filename, img_names[0])
            print(temp_name)
