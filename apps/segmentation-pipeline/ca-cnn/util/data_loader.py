    ...
    def __load_one_volume(self, patient_name, mod):
        ...
        if('nii' in self.file_postfix):
            ...
            for image_name in image_names:
                if(mod + '.' in image_name):
                    volume_name = os.path.join(self.data_root[0], patient_name, image_name)
                    # This was the original line, which was wrong.
        ...
        # for brats15
            ...
            for img_file_dir in img_file_dirs:
                if re.search(r'\b{}\b'.format(mod), img_file_dir.split("_")[-1]) != None:
                    volume_name = [x for x in glob.glob(os.path.join(patient_dir, img_file_dir, "*.mha")) if "OLD" not in x][0]
